...
- 16 个视频作品（旧横屏成都片替换为《檐下听雨》，新增三条竖屏短片）
- 横屏、竖屏按真实画幅比例混合排版
- 全部 / 横屏 / 竖屏筛选
- 点击卡片进入全屏播放器
...
- MP4 优先从 jsDelivr 边缘缓存读取，失败自动回退到 GitHub Pages 原地址
- 所有新增 MP4 已压缩为 720p/30fps、H.264 Main、AAC、Fast Start；四条合计约 11.4MB
- 旧封面统一压缩到 720/960px，封面总量从约 4.1MB 降到约 1.1MB
...
- 视频播放器优先使用 jsDelivr 的边缘缓存，并保留 GitHub Pages 回退；这样国内访问不依赖单一线路。
- 若后续接入对象存储，只需替换 `assets/data.js` 中的 `MOTION_ASSET_CDN`，播放器逻辑无需改动。
